...
group.value.fallback.action ??= 'SEND'
...
const fallbackAction = computed({ get: () => group.value.fallback.action ?? 'SEND', set: value => { group.value.fallback.action = value } })
...
const status = (key: string) => { const trace = traceFor(key); return trace ? key === group.value.fallback.id && trace.state === 'matched' && fallbackAction.value === 'SKIP' ? '不发送' : { matched: '命中', unmatched: '未命中', skipped: '未执行' }[trace.state] : '待预览' }
...
<template>
  <el-dialog v-model="visible" class="template-rule-group-dialog-shell" title="条件模板 · 多分支设置" width="min(1080px, 90vw)" top="4vh" append-to-body :close-on-click-modal="false" :before-close="close">
    <div class="branch-guide"><el-icon><InfoFilled /></el-icon><span>分支按顺序匹配，命中后不再继续；全部未命中时按默认分支设置处理。</span></div>
    <div class="branch-layout">
      <aside class="branch-sidebar">
        ...
        <div class="branch-list">
          ...
          <div class="branch-row branch-row--fallback" :class="{ active: selectedId === group.fallback.id }"><button class="branch-select" @click="selectedId = group.fallback.id"><span class="branch-select__title"><strong>默认分支</strong><el-tag size="small" :type="traceFor(group.fallback.id)?.state === 'matched' ? 'success' : 'info'">{{ status(group.fallback.id) }}</el-tag></span><small>{{ fallbackAction === 'SKIP' ? '所有条件均未命中时不发送' : '所有条件均未命中时发送默认消息' }}</small></button></div>
        </div>
      </aside>
      <section :key="selected.id" class="branch-editor">
        ...
        <div v-else class="branch-editor__section branch-editor__section--action"><div class="branch-editor__section-title"><div><h4>未命中处理</h4><p>所有条件分支均未命中时执行</p></div></div><el-radio-group v-model="fallbackAction" class="fallback-action"><el-radio-button value="SEND">发送默认消息</el-radio-button><el-radio-button value="SKIP">不发送</el-radio-button></el-radio-group></div>
        <div v-if="version || fallbackAction === 'SEND'" class="branch-editor__section"><div class="branch-editor__section-title"><div><h4>完整消息正文</h4><p>命中该分支后输出的消息内容</p></div></div><TemplateRuleContent v-model="selected.content" :params="params" :lists="lists" /></div>
        <div v-else class="branch-editor__skip">所有条件均未命中时，系统将结束本次处理，不发送消息。</div>
...
.branch-editor { min-width: 0; min-height: 0; padding: 18px 20px 24px; overflow-y: auto; background: #fff; scrollbar-color: #cbd5e1 transparent; scrollbar-width: thin; &__name { display: grid; grid-template-columns: 92px minmax(0, 1fr); align-items: center; padding-bottom: 16px; label { color: #303b4f; font-size: 14px; font-weight: 600; } } &__section { padding: 16px; margin-bottom: 14px; border: 1px solid #e6ebf2; border-radius: 10px; background: #fbfcfe; &--action { background: #fff; } } &__section-title { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; h4, p { margin: 0; } h4 { color: #202b3c; font-size: 15px; } p { margin-top: 3px; color: #98a2b3; font-size: 12px; } } &__skip { padding: 14px 16px; border: 1px solid #dbeafe; border-radius: 8px; background: #f0f7ff; color: #64748b; font-size: 13px; } &__preview { margin-top: 14px; } }
